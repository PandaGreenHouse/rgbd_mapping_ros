...
float distanceFromPointToLine(Eigen::Vector3f &vOrigin, Eigen::Vector3f &vDir, Eigen::Vector3f &p);
...
void refineDepthMap(cv::Mat &depthMat, int filterSize, int nloop);
void blendImages(cv::Mat &targ, cv::Mat &src, cv::Mat &depthMap, int tx, int ty, int tz);
void smoothing(cv::Mat &image, int kernel_length, int mode, cv::Mat &dst);
